...
emcc -o build/zfp_wrapper.js zfp_wrapper.c --pre-js build/pre.js --post-js build/post.js -I .built/include \
    -Lbuilt/lib -lm -lzfp -g0 -O2 -s WASM=1 -s ALLOW_MEMORY_GROWTH=1 \
...
if [[ $(find build/zfp_wrapper.js -type f -size +10000c 2>/dev/null) ]]; then
    ...
    cd ../node_modules
    rm -f zfp_wrapper
    ln -s ../zfp_wrapper/build zfp_wrapper
else
    echo "Not found!"
    exit
fi